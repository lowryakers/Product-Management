...
// app/data is the deployed copy (Railway builds with app/ as the root, so the
// repo-root data/ folder is not present). Fall back to the repo root for local
// runs right after a rebuild of the dataset.
const DATA = [
...
async function main() {
  ...
  if (staffEmail) {
    ...
    if (staffGenerated) console.log(`    temp password: ${staffPassword}`);
  }
